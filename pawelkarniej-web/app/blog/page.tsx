    ...
    <div className="relative min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <main className="relative z-10 mx-auto w-full max-w-6xl px-4 pb-24 pt-20 sm:px-6">
        ...
        <header className="mb-14 mt-14 max-w-3xl">
          ...
          <p className="mt-6 text-lg leading-relaxed text-zinc-600 dark:text-zinc-400">Practical guides and case studies from building React Native apps, AI products, subscription systems, and App Store releases.</p>
        </header>
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {posts.map((post) => (
            <Link key={post.slug} href={`/blog/${post.slug}`} className="group flex h-full flex-col overflow-hidden rounded-2xl border border-black/10 dark:border-white/10 bg-black/[0.03] dark:bg-white/[0.03] transition-all hover:-translate-y-1 hover:border-emerald-400/30 hover:bg-white/[0.06]">
              ...
              <article className="flex flex-1 flex-col p-6">
                ...
                <h2 className="mt-3 text-xl font-semibold leading-snug text-zinc-900 dark:text-white transition-colors group-hover:text-emerald-300">{post.data.title}</h2>
                {post.data.excerpt ? <p className="mt-3 line-clamp-3 text-sm leading-relaxed text-zinc-600 dark:text-zinc-400">{post.data.excerpt}</p> : null}
